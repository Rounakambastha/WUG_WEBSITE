...
const SignIn: React.FC = () => {
  ...
  const { login, loginWithGoogle, loginWithFacebook, setupRecaptcha, signinWithPhone } = useAuth();
  ...
  useEffect(() => {
    // Initialize recaptcha when the component mounts just in case they use phone auth
    try {
      setupRecaptcha('recaptcha-container');
    } catch (e) {
      console.log('Recaptcha might already be initialized');
    }
  }, [setupRecaptcha]);
  ...
  const handleSocialLogin = async (provider: string) => {
    try {
      ...
      setErrors({ form: `Failed to login with ${provider}. ${error.message}` });
    }
  };
        ...
        <motion.div className="bg-white dark:bg-dark-surface rounded-2xl shadow-xl p-8" variants={itemVariants}>
          ...
          {isPhoneMode ? (
            <motion.div variants={itemVariants} className="space-y-6">
              <button onClick={() => setIsPhoneMode(false)} className="text-sm font-medium text-primary-500 hover:text-primary-600 mb-2 block">
                &larr; Back to Email/Social Login
              </button>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="123456"
                        maxLength={6}
                        className={`w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-dark-bg text-secondary-500 dark:text-dark-text tracking-widest text-center transition-all duration-200 ${errors.otp ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`}
                      />
                    </div>
