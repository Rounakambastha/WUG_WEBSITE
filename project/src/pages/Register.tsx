...
const Register: React.FC = () => {
  ...
  const { signup, loginWithGoogle, setUpRecaptcha, requestPhoneOTP, confirmPhoneOTP } = useAuth();
  ...
  const [authMode, setAuthMode] = useState<'email' | 'phone_request' | 'phone_verify'>('email');
  ...
  const handlePhoneRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.phoneNumber) return setErrors({ phone: 'Phone number is required' });
    try {
      if (!(window as any).recaptchaVerifier) {
        (window as any).recaptchaVerifier = setUpRecaptcha('recaptcha-container');
      }
      const result = await requestPhoneOTP(formData.phoneNumber, (window as any).recaptchaVerifier);
      setConfirmationResult(result);
      setAuthMode('phone_verify');
      setErrors({});
    } catch (err: any) {
      console.error(err);
      setErrors({ phone: 'Failed to send OTP. Ensure phone includes country code (e.g. +1).' });
    }
  };
  ...
  const handlePhoneVerify = async (e: React.FormEvent) => {
    ...
    try {
      await confirmPhoneOTP(confirmationResult, formData.otp);
      navigate('/');
    } catch (err: any) {
      console.error(err);
        ...
        <motion.div className="bg-white dark:bg-dark-surface rounded-2xl shadow-xl p-8" variants={itemVariants}>
          ...
          {authMode === 'email' ? (
            ...
            <form onSubmit={handlePhoneRequest} className="space-y-6">
              ...
              <motion.div variants={itemVariants}>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-dark-bg text-secondary-500 dark:text-dark-text transition-all duration-200 ${errors.phone ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`}
                  />
                </div>
              ...
              <motion.div variants={itemVariants}>
                <Button type="submit" variant="primary" size="lg" className="w-full mb-2">
                  Send OTP
                </Button>
                <Button type="button" variant="outline" size="lg" className="w-full" onClick={() => setAuthMode('email')}>
                  Back to Form
                </Button>
              </motion.div>
            </form>
            ...
            <form onSubmit={handlePhoneVerify} className="space-y-6">
              <motion.div variants={itemVariants}>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    placeholder="123456"
                    className={`w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-dark-bg text-secondary-500 dark:text-dark-text transition-all duration-200 ${errors.otp ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`}
                  />
                </div>
              ...
              <motion.div variants={itemVariants}>
                ...
                <Button type="button" variant="outline" size="lg" className="w-full" onClick={() => setAuthMode('phone_request')}>
                  Back
                </Button>
              </motion.div>
            </form>
